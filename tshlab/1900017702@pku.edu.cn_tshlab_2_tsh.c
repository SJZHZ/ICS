...
1900017702朱越
改写了eval（并增加了builtin_command和waitfg）和三个处理程序
...
void  eval(char *cmdline) 
{
    ...
    if (!builtin_command(&tok)) //如果不是内置命令，交给子进程运行
    {
        sigset_t mask, prev, all, hup;
        sigfillset(&all);
        sigemptyset(&mask);
		sigemptyset(&hup);
        ...
        sigprocmask(SIG_BLOCK, &all, &prev);//先屏蔽信号
        if ((pid = fork()) == 0)
        {
            sigprocmask(SIG_SETMASK, &prev, NULL);//解除屏蔽
            setpgid(0, 0);//每次都要新建一个进程组

            //I/O重定向
            int inf, outf;
            if (tok.infile != NULL)//input reset
            {
                inf = open(tok.infile, O_RDWR, 0);
                dup2(inf, 0);
            }
            if (tok.outfile != NULL)//output reset
            {
                outf = open(tok.outfile, O_RDWR, 0);
                dup2(outf, 1);
            }
                execve(tok.argv[0], tok.argv, environ);//子进程离开
        }

        sigprocmask(SIG_BLOCK, &all, NULL);//先阻塞所有信号
        int state = 1 + bg;//FG = 1, BG = 2
        ...
        sigprocmask(SIG_UNBLOCK, &hup, NULL);//unblock hup
    }
    return;
}

//parseline作用是解析命令行并设置tok，返回值区分前后台，无需改动
/* 
...
//有quit, nohup, jobs, kill, fg, bg
int builtin_command(struct cmdline_tokens *tok)
{
    if (tok->builtins == BUILTIN_QUIT)//quit
        exit(0);
    if (tok->builtins == BUILTIN_NOHUP)//nohup, write Later
	{
		sigset_t hup;
		sigemptyset(&hup);
		sigaddset(&hup, SIGHUP);
...
void waitfg(pid_t pid)//子进程在前台执行=等待子进程执行完毕再结束
{
	sigset_t emp;
	flag = 0;//设置标记位，等待改变
	sigemptyset(&emp);
 ...
 */
void sigchld_handler(int sig) //回收子进程（尽可能多的地）或处理stop
{
    int olderrno = errno;
    sigset_t all, prev;
    sigfillset(&all);
	...
	{
        sigprocmask(SIG_BLOCK, &all, &prev);
		if (pid == fgpid(job_list))//如果是前台作业
			flag = 1;		//回收了就可以结束wait了
		job = getjobpid(job_list, pid);
        int jid = job->jid;

		if (WIFEXITED(status))//退出
...
{
    sigset_t all, prev;
    sigprocmask(SIG_BLOCK, &all, &prev);//尽早屏蔽总是好的
